// MenuPage.tsx — v3
// Consomme RestaurantConfig via useRestaurantConfig
// Chaque section lit sa portion de config pour adapter le rendu

import { useEffect, useState, useCallback, useMemo, useRef } from "react";
import { AnimatePresence, motion, useScroll, useTransform } from "framer-motion";
import { createClient } from "@supabase/supabase-js";
import { MapPin, Phone, Globe, MessageCircle, Music2, UtensilsCrossed, Search, X } from "lucide-react";
...
// ─────────────────────────────────────────────────────────────
// StyledBackground — lit config.background
// ─────────────────────────────────────────────────────────────

function StyledBackground({ colors, config }: {
  colors: Colors;
  config: ReturnType<typeof useRestaurantConfig>;
}) {
  const { patternTop, patternBottom, patternTopOpacity, patternBottomOpacity, blobTopLeft, blobBottomRight } = config.background;
...
// ─────────────────────────────────────────────────────────────
// DynamicHero — lit config.header
// ─────────────────────────────────────────────────────────────

function DynamicHero({ restaurant, colors, config }: {
  ...
  const arcStyle: React.CSSProperties = {
    ...
    borderRadius: h.arcShape === "round" ? "0 0 50% 50% / 0 0 80px 80px" : "0",
    zIndex: 0,
  };
  ...
  const content = (
    <motion.header
      ref={heroRef}
      style={h.parallax
        ? { opacity, y, position: "relative", overflow: "hidden", paddingBottom: 0 }
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "clamp(36px,8vw,64px) 24px clamp(48px,10vw,80px)", textAlign: "center" }}>
        ...
        <div style={{ position: "relative", width: 90, height: 90, margin: "0 auto 20px" }}>
          {h.logoPulse && (
            <div style={{ position: "absolute", inset: -8, borderRadius: "50%", border: `2px solid ${colors.primary}40`, animation: "pulse-ring 2.4s ease-out infinite" }} />
          )}
          <motion.div
            initial={{ scale: 0.6, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ delay: 0.1, type: "spring", stiffness: 280, damping: 20 }}
            style={{ width: 90, height: 90, borderRadius: "50%", border: `3px solid ${colors.primary}30`, background: "rgba(255,255,255,0.12)", backdropFilter: "blur(12px)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}
          >
            {restaurant.logo_url
        ...
        {h.showBigMenu ? (
          <motion.div
            ...
            style={{ fontSize: "clamp(64px, 20vw, 108px)", lineHeight: 0.82, fontWeight: 900, letterSpacing: "-0.04em", color: "#fff", fontFamily: restaurant.font_display ?? "Georgia, serif", userSelect: "none", textShadow: `0 6px 30px ${colors.accent}60` }}
          >
            ME<br />NU
          </motion.div>
        ) : (
          <motion.div
            ...
            style={{ fontSize: "clamp(32px, 10vw, 56px)", fontWeight: 900, letterSpacing: "-0.03em", color: "#fff", fontFamily: restaurant.font_display ?? "Georgia, serif" }}
          >
            {h.heroTitle ?? restaurant.name}
          </motion.div>
        )}
        ...
        <motion.h1
          ...
          style={{ marginTop: 14, fontSize: "clamp(15px, 4vw, 22px)", letterSpacing: "0.18em", color: "rgba(255,255,255,0.9)", fontFamily: restaurant.font_display ?? "Georgia, serif", fontWeight: 400 }}
        >
          {restaurant.name}
        </motion.h1>
...
function SearchBar({ query, setQuery, colors, resultCount }: {
  query: string;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.4 }} style={{ padding: "12px 16px 4px" }}>
      <div style={{ position: "relative", display: "flex", alignItems: "center", background: "rgba(255,255,255,0.07)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", border: `1.5px solid ${query ? colors.primary + "40" : colors.primary + "18"}`, borderRadius: 16, padding: "0 14px", transition: "border-color 0.25s", boxShadow: query ? `0 4px 20px ${colors.accent}25` : "none" }}>
        ...
        <input ref={inputRef} type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher un plat…"
        ...
        <AnimatePresence>
          {query && (
            <motion.div initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.7 }} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <motion.button whileTap={{ scale: 0.85 }} onClick={() => { setQuery(""); inputRef.current?.focus(); }}
...
function FunSocials({ restaurant, colors, config }: {
  restaurant: Restaurant;
  colors: Colors;
  config: ReturnType<typeof useRestaurantConfig>;
}) {
  ...
  const socials = [
    ...
    ...(restaurant.phone ? [{ key: "phone", label: "Appeler", emoji: "📞", bg: "#555", Icon: Phone, href: `tel:${restaurant.phone}` }] : []),
  ];
    ...
    >
      ...
      <div style={gridStyle}>
        {socials.map(({ key, label, emoji, bg, Icon, href }, idx) => (
          <motion.a
            key={key} href={href} target="_blank" rel="noreferrer"
...
export default function MenuPage({ slug }: MenuPageProps) {
  const config = useRestaurantConfig(slug);  // ← config du resto
    ...
    <>
      ...
      <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", width: "100%", overflow: "visible" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", width: "100%" }}>
          ...
          <DynamicHero restaurant={restaurant} colors={colors} config={config} />

          {/* Sticky : SearchBar + CategoryNav */}
          <div style={{ position: "sticky", top: 0, zIndex: 20, background: `${colors.bg}d0`, backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: `1px solid ${colors.primary}10`, boxShadow: `0 4px 24px ${colors.bg}80` }}>
            <SearchBar query={searchQuery} setQuery={setSearchQuery} colors={colors} resultCount={totalResults} />
            <AnimatePresence>
              {!searchQuery && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.28 }} style={{ overflow: "hidden" }}>
                  <CategoryNav colors={colors} activeCats={activeCats} activeCategory={activeCategory} setActive={setActive} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
                ...
                filteredCats.map(cat => {
                  ...
                  return (
                    <CategorySection
                      ...
                      items={catItems}
                      colors={colors}
                      config={config.card}   // ← config des cartes
          ...
          <FunSocials restaurant={restaurant} colors={colors} config={config} />
